...
public class AsyncConfig extends AsyncConfigurerSupport {
	 ...
	 */
	@Bean(name = "clientMainTaskJob")
	public Executor clientMainTaskJob() {
		ThreadPoolTaskExecutor executor = new ThreadPoolTaskExecutor();
		executor.setCorePoolSize(1);
		executor.setMaxPoolSize(1);
		executor.setQueueCapacity(1);
		executor.setThreadNamePrefix("clientMainTaskJob-");
		executor.initialize();
		return executor;
	}
	 ...
	 */
	@Bean(name = "clientJobTaskJob")
	public Executor clientJobTaskJob() {
		ThreadPoolTaskExecutor executor = new ThreadPoolTaskExecutor();
		executor.setCorePoolSize(3);
		executor.setMaxPoolSize(3);
		executor.setQueueCapacity(0);
		executor.setThreadNamePrefix("clientJobTaskJob-");
		executor.initialize();
		return executor;
	}
}
